...
public sealed partial class JustSpawnerUI : FancyWindow
{
    public event Action<bool, int, bool, string, bool, string>? OnConfirm;

    private readonly List<JustSpawnerCharacterInfo> _characters = new();
    ...
    public void SetData(JustSpawnerBuiState state)
    {
        _characters.Clear();
    ...
    private void UpdateFields(int id)
    {
        ...
        if (!CustomNameCheck.Pressed)
            CustomNameInput.Text = info.Name;
        if (!CustomDescriptionCheck.Pressed)
    ...
    {
        UseRandomCharacter.Text = Loc.GetString("just-spawner-button-random", ("timer", _timeLeft));
    }
